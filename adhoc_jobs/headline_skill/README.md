...
```bash
python adhoc_jobs/headline_skill/scripts/hard_filter.py \
  --profile adhoc_jobs/headline_skill/profiles/default_xiaohongshu.yaml \
...
```bash
python adhoc_jobs/headline_skill/scripts/rhythm_scorer.py \
  --profile adhoc_jobs/headline_skill/profiles/default_xiaohongshu.yaml \
  --input candidates.json
```
...
```bash
python adhoc_jobs/headline_skill/scripts/evaluate_candidates.py \
  --profile adhoc_jobs/headline_skill/profiles/default_xiaohongshu.yaml \
  --input candidates.json
```
...
```bash
python adhoc_jobs/headline_skill/scripts/validate_skill.py adhoc_jobs/headline_skill
python -m unittest discover -s adhoc_jobs/headline_skill/tests -p "test_*.py"
```

## GitHub Actions

This package ships with `.github/workflows/headline-skill-ci.yml`.

The workflow runs on pushes, pull requests, and manual dispatch when files under `adhoc_jobs/headline_skill/` change. It checks:
