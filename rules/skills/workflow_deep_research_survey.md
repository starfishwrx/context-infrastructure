...
### 0.5: 实体与内部标签校准（公司 / 招聘 / 访谈类调研时优先执行）

**触发条件**：调研对象涉及公司、团队、岗位、访谈对象或招聘流程，且材料中出现“某某工作室”“某业务线”“某内部项目组”这类可能来自 recruiter、JD、采访转述或二手整理的内部标签。

**核心原则**：把这类内部组织名先视为**待验证假设**，不要直接当作事实复述。很多标签只在招聘或沟通语境里成立，不一定对应公开可核验的正式实体、事业群或 legal unit。

**操作**：

1. 把内部标签单独列成 hypothesis，而不是事实结论。
2. 优先寻找可公开验证的锚点：官方招聘页、公司官网、工商/备案主体、财报/公告、产品页、应用商店发行主体。
3. 如果标签本身无法独立验证，就退回到“可验证的 workflow / 职责 / 协作链路”来描述对象，不重复未证实标签。
4. 用相邻角色 JD、协作岗位描述、公开业务动作和组织边界反推真实工作流，再明确标注哪些部分是重建判断，哪些部分是公开事实。

**推荐产出**：在 `tmp/<session_slug>/scratchpad.md` 加一个小表，避免后续写作时把假设写成事实：

```markdown
## Entity / Label Calibration

| 内部标签或说法 | 状态 | 公共锚点 | 处理方式 |
|---|---|---|---|
| “InJoy 工作室” | 待验证 | 官方招聘页仅写“发行-境外市场” | 报告中改写为“吉比特境外发行相关团队” |
| “某 AI 中台” | 已验证 / 未验证 | 官网产品页 / 财报 / 工商主体 | 决定是否可直接复述 |
```

**写作约束**：

- 未验证标签不要进入最终结论句，除非明确标注为“招聘或沟通语境中的称呼”。
- 可以重建 workflow，但必须把“公开事实”和“基于相邻证据的推断”分开写。
- 当公开锚点和内部标签冲突时，以公开锚点为准。
...
同时启动 3-5 个 sub-agent，每个负责一个维度。具体调用方式见 `workflow_parallel_subagents.md`，先按当前 harness 可用工具分流：

- OpenCode：使用 `multi_tool_use.parallel` 包多个 `functions.task` 调用。默认用 `general`；低成本初筛可用 `cheap_glm`；高隐私材料用 `private_ds4`；需要 zero-data-retention 云模型时按任务强度选择 `ollama_kimi` 或 `ollama_deepseek_pro`；复杂判断或最终 QA 用 `reasoning_gpt`。
- Codex Studio：如果没有 `functions.task`，先用 `tool_search` 搜索 `multi-agent subagent task spawn agent`。若发现 `multi_agent_v1.spawn_agent`，并且用户已显式要求 Multi-Agent、并行调研、delegation 或 subagent，则用多个 `spawn_agent` 启动并行 agent；通用调研用 `agent_type: default`，代码库探索用 `agent_type: explorer`。不要照抄 OpenCode 的 `subagent_type` 名称，也不要随意指定 `model`。
- 无可用 subagent 工具，或 Codex Studio 当前工具策略不允许自动 spawn：主线程完成调研，并在 `search_manifest.md` 记录回退原因。
...
{
  "agent_type": "default",
  "message": "你负责调研 XX 维度。请返回关键结论、URL、原文摘录、冲突点和需要主线程复核的地方。"
}
```
