...
Sub-agent 调度遵循 [并行 Subagent 工作流](./workflow_parallel_subagents.md) 的规则：并行度 ≤5，调研 overlap 30-50%。按当前 harness 暴露的 subagent schema 并行派发；OpenCode 下使用 `multi_tool_use.parallel` 包多个 `functions.task`。
