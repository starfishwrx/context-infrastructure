...
## Demo 应展示工作流，不要只展示模型功能

### 常见误区

很多 AI demo 会直接围绕"生成、改写、总结、打分"这类模型能力组织页面和文案，结果是：

- 观众只能看到模型接口拼装，难以理解真实使用场景
- 产品边界模糊，看起来像"又一个通用 AI 工具"
- 作品集叙事停留在 feature list，无法证明业务判断和流程设计能力

### 更稳的 framing

优先把 demo 表达成一个端到端工作流产品，而不是一个模型能力展示页。

推荐做法：

- 先定义用户真正要完成的阶段性任务，而不是先列模型功能
- 把采集、筛选、生成、编辑、发布、复盘等操作状态显式化
- 用有限但连贯的模拟交互证明系统如何推进一次完整任务闭环
- 文案重点放在流程约束、判断节点、输入输出关系，而不是"AI 很强"

### 适用判断

当产品目标包含作品集展示、面试 case、原型验证或对外演示时，这条原则尤其重要。评估者真正要判断的，通常不是模型能不能输出一段文本，而是你是否能把 AI 嵌进一个清晰、可信、可执行的业务流程。
...
| 2026-05-17 | 补充“demo 以端到端工作流 framing，而非模型能力堆砌”的设计原则 |
